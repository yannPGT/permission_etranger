...
- Demandes : autoriser les lectures selon la matrice. Écriture auteur uniquement pour `BROUILLON` ou `A_CORRIGER`, et interdire Unite/Entite/CreeePar/Version/Statut en écriture directe. `PiecesJointes` est modifiable par l’auteur en brouillon/correction, par l’acteur assigné aux étapes PDF et par l’administrateur. `VersionPDFActive` n’est modifiable que pendant une transition autorisée. Les autres transitions sont réservées au rôle responsable.
- Actions : lecture si la demande est visible ; ajout/modification par rôle assigné ou admin. Interdire la modification après traitement. Les colonnes `VersionPDFEntree`, `TraitementPDF`, `VersionPDFSortie`, `CommentairePDF`, `DateValidationPDF` et `ValidationPDFPar` ne sont modifiables que par l’acteur assigné à une action `EN_COURS` ou par l’administrateur agissant en supervision.
