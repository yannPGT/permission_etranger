...
| Actions | Demande Ref:Demandes requis ; Etape Ref:EtapesWorkflow requis ; VersionDemande Int requis ; AssigneeA Ref:Personnel ; RoleAssigne Ref:Roles ; StatutAction Choice incluant `A_FAIRE`/`EN_COURS`/`TRAITEE`/`RETOURNEE`/`ANNULEE` ; DateTransmission/DateAccuseReception/DatePriseEnCharge/DateTraitement DateTime ; Decision Choice incluant `ANNULEE` ; MotifRetour Text obligatoire pour RETOUR ; Commentaire Text ; PiecesJointes Attachments ; TraiteePar Ref:Personnel ; VersionPDFEntree Ref:VersionsPDF ; TraitementPDF Choice `SANS_MODIFICATION`/`NOUVELLE_VERSION` ; VersionPDFSortie Ref:VersionsPDF ; CommentairePDF Text ; DateValidationPDF DateTime ; ValidationPDFPar Ref:Personnel ; PDFPretPourDecision Bool formule ; CleMigration Text unique ; UrlResanaHistorique Text sensible |
| Historique | Demande Ref:Demandes requis ; Version Int ; DateHeure DateTime requis ; Utilisateur Ref:Personnel ; TypeEvenement Choice requis, incluant `MODIFICATION_DEMANDE` et `ANNULATION` ; AncienStatut/NouveauStatut Ref:Statuts ; AncienneEtape/NouvelleEtape Ref:EtapesWorkflow ; Commentaire Text ; ResumeModification Text. Ajout seul, aucune modification/suppression hors procédure administrateur. |
...
Le référentiel `Statuts` doit contenir `ANNULEE` avec `Terminal = TRUE`. Lors d’une annulation, les actions encore `A_FAIRE` ou `EN_COURS` passent à `ANNULEE`, la demande est clôturée et un événement `ANNULATION` conserve le motif et l’auteur. Cette valeur est distincte de `REFUSEE`, qui correspond à une décision définitive d’un valideur.
